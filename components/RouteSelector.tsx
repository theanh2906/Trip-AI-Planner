...
const RouteSelector: React.FC<RouteSelectorProps> = ({ routes, onSelectRoute, isLoading, language }) => {
  ...
  return (
    ...
    <div className="absolute top-[240px] md:top-80 left-0 md:left-4 right-0 md:right-auto px-4 md:px-0 z-10 w-full max-w-md animate-fade-in-up">
      <div className="flex items-center justify-between mb-3 px-1">
        <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider">{t.selectRoute}</h3>
        <span className="text-xs bg-slate-200 text-slate-600 px-2 py-1 rounded-full">{routes.length} {t.options}</span>
      </div>
        ...
        {routes.map((route) => (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-2 pl-2">
              <h4 className="font-bold text-slate-800 text-lg group-hover:text-blue-600 transition-colors">{route.name}</h4>
