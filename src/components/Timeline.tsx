...
const Timeline: React.FC<TimelineProps> = ({
  ...
  return (
    <>
      ...
      <div
        className={`absolute inset-y-0 left-0 bg-white shadow-2xl z-20 flex-col pointer-events-auto ${mobileClasses} ${desktopClasses}`}
      >
          ...
          <div className="absolute inset-0 z-50 bg-white animate-fade-in flex flex-col pointer-events-auto">
            <div className="relative h-64 w-full flex-shrink-0">
              ...
              <div className="absolute bottom-6 left-6 right-6 text-white">
                <div
                  className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold mb-2 backdrop-blur-md bg-white/20 text-white border border-white/20`}
                >
                  <CategoryIcon type={selectedItem.type} className="w-3 h-3" />
                  {selectedItem.type}
                </div>
                <h2 className="text-3xl font-bold leading-tight shadow-black drop-shadow-lg">
                  {selectedItem.title}
                </h2>
              </div>
            </div>
